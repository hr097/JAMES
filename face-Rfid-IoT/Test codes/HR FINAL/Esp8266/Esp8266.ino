...
#define Response_status 12 // send response

// except: [0 2] all GPIO [16] is free 
...
{
  Serial.println(" ");
  Serial.println("Waiting for Receving Tag ID from Arduino UNO R3...");
...
void loop() {
   ...
   {    
        ...
        /* Send Received ID to API Server for validation */
         ...
         Serial.println("RFID :"+Rfid_tag_ID);
...
//         );
         ...
         String temp = String("POST ") + url + " HTTP/1.1\r\n" +
         "Host: " + host + "\r\n" +
         "User-Agent: JAMES 4.0\r\n" +
         "Content-length: 22\r\n"
         "Content-Type: application/json\r\n" +
         ...
         Serial.println(String("Server Response : " + Api_Response));  //HTTP/1.1 200 OK ..... { name : "firstname lastname"} All Response Given by API server in single String
        
         if(Api_Response.startsWith("HTTP/1.1 200 OK")==true) // validating  Response from Server with HTTP/1.1 200 OK
         {     
               ...
               forwardResponse(Rfid_tag_ID+"==OK");
