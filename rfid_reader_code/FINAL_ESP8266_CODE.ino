...
//@change pin
#define Rfid_status 5          //D1 D2(32)  - Rfid read status [Blue light]   
#define Attendance_status 4    //D2 D4  - Attedance Verified status [Green light]
#define Invalid_card_status 0 //D3 D15 - Invalid card scanned / failure (verification of attedance) [Red light] 
#define Internet_status 2      //D4     D5  - System connection with Active Internet connection
...
String readerNumber = "2"; // @change Reader Number
...
char WIFI_SSID[3][30] = {"reader02","MscIT", "VNSGU"}; // Wifi ssid array to store multiple wifi because in case of one connection failure system can auto connect with another wifi available
char WIFI_PASSWORD[3][30] = {"reader02","mscit@2020", "mscit@2020"}; // ssid -password mapped
...
const char *serverName = "http://ams.vnsguit.org/api/addstudatt.php"; // Domain name with full URL path to an api for HTTP POST Request
...
//        digitalWrite(control_status, LOW); //set intial status
        ...
        Serial.println(" ");
        Serial.println("Esp8266 is ready to serve :)"); // system online debug message
        Serial.println(" ");
    }
...
{    
   //@change api token
    ...
    if(httpResponseCode == 200 && payload=="1") //@change logic to only 1
    {   
        Serial.println(" ");
