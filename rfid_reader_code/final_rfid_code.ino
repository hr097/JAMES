...
#define Internet_status 21       // D5  - System connection with Active Internet connection (white light)
//#define control_status 12      // D13 - System control pin ESP32 & READER Online/Offline (yellow light)
...
String readerNumber = "1"; // Reader Number
...
// wifi configuration
char WIFI_SSID[3][30] = {"reader01","MscIT", "VNSGU"};              // Wifi ssid array to store multiple wifi because in case of one connection failure system can auto connect with another wifi available
char WIFI_PASSWORD[3][30] = {"reader01","mscit@2020", "VNSGU"}; // ssid -password mapped
...
void loop()
{
    if (WiFi.status() == WL_CONNECTED) // if wifi gets connected
    {
        ...
        if (nfc.tagPresent()) // Look for new rfid card tag
        {
            NfcTag tag = nfc.read();

            Rfid_tag_ID = tag.getUidString();
