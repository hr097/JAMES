...
{    
    ...
    echo "<br>".ini_set('display_errors',0); // for display error
    echo "<br>".ini_set('display_startup_errors',0); // for display startup error
    echo "<br>".ini_set('file_uploads',1); // turn on file upload
    echo "<br>".ini_set('allow_url_include',0); // for allowing external link http/https files with include/require
    echo "<br>".ini_set('mysqli.reconnect',1); // recomend the MYSQL
    echo "<br>".ini_set('mysqli.rollback_on_cached_plink',1); //rollback changes in db when connection is half closed
